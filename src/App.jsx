...
<button onClick={()=>{if(listening&&recogRef.current){recogRef.current.stop();return}if(!('webkitSpeechRecognition' in window||'SpeechRecognition' in window)){alert('Speech not supported in this browser');return}const SR=window.SpeechRecognition||window.webkitSpeechRecognition;const r=new SR();r.lang='en-GB';r.continuous=true;r.interimResults=true;recogRef.current=r;let final='';r.onstart=()=>setListening(true);r.onend=()=>{setListening(false);recogRef.current=null;if(final)setAnswer(prev=>prev?prev+' '+final:final)};r.onresult=e=>{let f='',i='';for(let x=0;x<e.results.length;x++){if(e.results[x].isFinal)f+=e.results[x][0].transcript;else i+=e.results[x][0].transcript}final=f;setAnswer(prev=>{const base=prev.replace(/\s*\.{3}.*$/,'');const combined=base?(base+' '+f+i).trim():(f+i).trim();return combined})};r.onerror=()=>{setListening(false);recogRef.current=null};r.start()}} style={{...b("transparent",listening?"#ef4444":"#94a3b8"),border:`1px solid ${listening?"#ef444444":"#33415544"}`,padding:"12px 16px",animation:listening?"pulse 1s infinite":"none"}}>{listening?"⏹":"🎤"}</button> <button onClick={()=>{if(!('webkitSpeechRecognition' in window||'SpeechRecognition' in window)){alert('Speech not supported in this browser');return}const SR=window.SpeechRecognition||window.webkitSpeechRecognition;const r=new SR();r.lang='en-GB';r.interimResults=false;r.onstart=()=>setListening(true);r.onend=()=>setListening(false);r.onresult=e=>{const t=e.results[0][0].transcript;setAnswer(prev=>prev?prev+' '+t:t)};r.onerror=()=>setListening(false);r.start()}} style={{...b("transparent",listening?"#ef4444":"#94a3b8"),border:`1px solid ${listening?"#ef444444":"#33415544"}`,padding:"12px 16px",animation:listening?"pulse 1s infinite":"none"}}>{listening?"⏺":"🎤"}</button> <button onClick={()=>{if(!('webkitSpeechRecognition' in window||'SpeechRecognition' in window)){alert('Speech not supported in this browser');return}const SR=window.SpeechRecognition||window.webkitSpeechRecognition;const r=new SR();r.lang='en-GB';r.interimResults=false;r.onstart=()=>setListening(true);r.onend=()=>setListening(false);r.onresult=e=>{const t=e.results[0][0].transcript;setAnswer(prev=>prev?prev+' '+t:t)};r.onerror=()=>setListening(false);r.start()}} style={{...b("transparent",listening?"#ef4444":"#94a3b8"),border:`1px solid ${listening?"#ef444444":"#33415544"}`,padding:"12px 16px",animation:listening?"pulse 1s infinite":"none"}}>{listening?"⏺":"🎤"}</button></div>
              </div>
            ):(
